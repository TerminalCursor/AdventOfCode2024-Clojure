...
(defn parse-row [row]
  (def row-len (count row))
  (def result (for [n (range 0 row-len)]
                (do
                  (def subvector (concat (subvec row 0 n) (subvec row (+ n 1))))
                  (parse-row-part subvector))))
  (> (count (filter #(= % true) result)) 0))
...
;(def response (aoc-post/post-answer 2024 2 2 safe-count))
